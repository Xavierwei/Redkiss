...
MobileEsp.InitDeviceScan(), ! function (e) {
    ...
    return "string" == typeof n && (n = {
      ...
      if (a.playlist.length) {
        ...
        l.length && l.replaceWith(p = e("<p />")), l = e("<video />").addClass("fp-engine"), p ? p.replaceWith(l) : r.prepend(l), flowplayer.support.video && l.attr("preload", m), "string" == typeof a.playlist[0] ? l.attr("src", a.playlist[0]) : e.each(a.playlist[0], function (n, t) {
          for (var o in t)
            t.hasOwnProperty(o) && l.append(e("<source />").attr({
              type: "video/" + o,
              src: t[o]
            }))
        }), c = new s(l)
      ...
      y.conf = e.extend(y.conf, a), e.each(["bind", "one", "unbind"], function (e, n) {
        ...
        }), d.push(y), y.engine ? (a.splash ? y.unload() : y.load(), a.disabled && y.disable(), i.volume(y.volumeLevel), void r.one("ready", t)) : y.trigger("error", {
  ...
  flowplayer.engine.html5 = function (n, o) {
    ...
    return s = {
      ...
      load: function (t) {
        if (p.splash && !c)
          d = y(t).prependTo(o), u.inlineVideo || d.css({
            position: "absolute",
            top: "-9999em"
...
function () {
  ...
  for (var m = ["log", "info", "warn", "error", "debug", "trace", "dir", "group", "groupCollapsed", "groupEnd", "time", "timeEnd", "profile", "profileEnd", "dirxml", "assert", "count", "markTimeline", "timeStamp", "clear"], i = 0; i < m.length; i++)
  ...
  ! function (Interface) {
    var Loader = function () {
      ...
      return Loader.prototype.start = function () {
        ...
        //                $(this.el).show();
        //                var p;
        //                this.target ? (p = Math.round(100 * (this.current / this.target)), p = "CHARGEMENT " + String(p) + "%") : p = "Loading…", $(this.el).text(p)
        //            },
        Loader.prototype.update = function () {
    ...
    var ImageFrameSource = function () {
      function ImageFrameSource(config) {
        ...
        for (var i = 0; this.length > i; i++) {
          ...
          //zzz
        for(var i = 0, length = this.group.length; length > i; i++) {
            this.groupFrames[i] = [];
            if(!(this.group[i][0] instanceof Array)) {
                var begin = this.group[i][0];
                var end = this.group[i][1];
                for(var j = 0; end - begin >= j; j++){
                    this.groupFrames[i].push(begin + j);
                }
            }
            else {
                for(var j = 0, kLength = this.group[i].length; kLength > j; j++ ) {
                    var jBegin = this.group[i][j][0];
                    var jEnd = this.group[i][j][1];
                    this.groupFrames[i][j] = [];
                    for(var k = 0; jEnd - jBegin >= k; k++){
                        this.groupFrames[i][j].push(jBegin + k);
                    }
                }
            }
            this.reSection[i] = [];
        ...
        this.urlFunc = function (n, prefix) {
          ...
          return SH.Config.isMobile ? fileDir = "/small_256/" : SH.Config.isTablet && (fileDir = "/small_512/"), config.path + "/large/" + config.prefix + SH.Util.zeroPad(n, config.frameNumberTemplate || "" + n) + "." + config.extension
        },
        this.largeUrlFunc = function (n, prefix) {
          //                    var fileDir = "PICS/large/";
          var fileDir = "PICS/large/";
          return SH.Config.isMobile ? fileDir = "/large_960/" : SH.Config.isTablet && (fileDir = "/large_960/"), config.path + "/large/" + config.prefix + SH.Util.zeroPad(n, config.frameNumberTemplate || "" + n) + "." + config.extension
      ...
      return ImageFrameSource.prototype.loadFrame = function (n, immediately) {
          ...
          // 已经加载完的帧在 frameLoads[]里面
          if (SH.Config.AFramesOnly && (n = n >> 1 << 1), this.frameLoads[n]) {
            return this.frameLoads[n]; // 如果已经加载完，直接返回
          }

          var i = new Image;
          this.frameLoads[n] = $.Deferred(),
          this.loadGauge.setLoading(n),
          i.onload = function () {
            _this.frames[n] = i,
        ...
        ImageFrameSource.prototype.appendToCustomSequence = function (begin,end) {
          console.log('appendToCustomSequence(',begin,', ',end,')');
          var len = end - begin;
          console.log('len: ',len);
          console.log('this.FLAG_customSequenceLast: ',this.FLAG_customSequenceLast + 1);
          for (var i = 1; len > i; i++) {
            // if(this.frameLoads[i]) {
            //   this.frameLoads[i].seq = i;
            // }
            this.customSequence[this.FLAG_customSequenceLast + i] = begin + i;
            // this.customSequence.push(begin + i);
          }

          this.FLAG_customSequenceLast += len;
          // this.customSequence.push.apply(this.customSequence, arr)
          redkiss.stream.acceptingInput = true
          return true;
        },
         ImageFrameSource.prototype.load = function (loader, startFrame) {
          // debugger; //此时FULL_DROP一张也没有下载
          var _this = this;
          ...
          for (var firstPhasePreloads = [], counter = 0, counterBack = 0, l = this.length, incrExtra = 0, incrBack = 0; l > counter; counterBack++, counter++) {
            ...
            counter += incrExtra,
            counterBack += (incrBack + 1) * reverseDecay - 1;
            ...
            if (_.each([idxF, idxB], function (idx_) {
              if (inBounds(idx_)) {
                ...
                console.log('firstPhasePreloads: ',firstPhasePreloads);
                break
            }
          }
          // console.log("preloading firstPhasePreloads.length: " + firstPhasePreloads.length);
          var loadMore = function () {
            // debugger;
            var nextLoads = [];
            if (!_this.allFramesLoading()) {
              // startFrame:  0,  _this.length:  1473
              for (var idx = startFrame, l = _this.length, firstEmpty = false; l > idx; idx += 2) {
              ...
              if (startFrame > 0) {
                // debugger;
                for (var idx = startFrame, firstEmpty = false; idx >= 0; idx -= 2) {
                  if (!firstEmpty) {
          ...
          return SH.Util.whenAll(firstPhasePreloads).then(function () {
            ...
            // debugger;
            // console.log('firstPhasePreloads success');
            // debugger; // 此时下载完成的: FULL_DROP 0到39张按顺序下载,39之后按2,4,6,8...间隔下载至1445
            // 此时firstPhasePreloads: Array[124]
            loadMore(),
            //                        $(".static .rotation-prompt").transition({
            //                            marginTop: 0
            //                        }),
            //                        $(".static .rotation-prompt").hide().css('marginTop')

            //                            $(".static .rotation-prompt").css('top',function() {
            //                                var self_height = $(".static .rotation-prompt").height();
            //                                var window_height = $(window).height();
            //                                var marginTop = -((window_height - self_height) / 2);
            //                                return marginTop + 'px';
            //                            })
          ...
          //console.log('getFrame(',n,')');
          //var num = this.customSequence[n];
          var num = n;
          ...
          // show scroll button
          if(num >= globalData.initFrameStart && num <= globalData.initFrameEnd && !$('.btn_scrolldown').is(':visible')) {
              $('.btn_scrolldown').show();
          }
          else if((num < globalData.initFrameStart && $('.btn_scrolldown').is(':visible')) || (num > globalData.initFrameEnd && $('.btn_scrolldown').is(':visible'))) {
              $('.btn_scrolldown').hide();
          }

          // hide nav
          if((num >= globalData.navFrameStart && num <= globalData.navFrameEnd && $('.nav-bar').is(':visible')) || (num >= globalData.navFrameStart2 && num <= globalData.navFrameEnd2 && $('.nav-bar').is(':visible'))) {
              $('.nav-bar').hide();
          }
          else if( (num < globalData.navFrameStart || num > globalData.navFrameEnd2 || (num > globalData.navFrameEnd && num < globalData.navFrameStart2)) && !$('.nav-bar').is(':visible') ) {
              $('.nav-bar').show();
          }
    ...
    ! function (Renderer) {
      ...
      var ImageToCanvas = function () {
        function ImageToCanvas(element) {
          this.element = element; // .viewport-inner
          this.prevImage = null;
          this.drawBox = null;
          this.aspect = null;
        ...
        return ImageToCanvas.prototype.handleResize = function (e, w, h, ww, wh) {
          // this function runs !
          //e: event
                 //console.log('handleResize(e: ', e, ', w: ', w, ', h: ', h, ', ww: ', ww, ', wh: ', wh);
          var _this = this;
          this.canvas.width = ww; // 窗口的宽
          this.canvas.height = wh; // 窗口的高
          ...
          this.resizeQueued || requestAnimationFrame(function () {
            ...
            //_this.canvas.style.marginTop = "" + marginTop + "px",
            _this.drawBox = [-marginLeft, -marginTop, w, h],
            _this.aspect = ww / wh,
            _this.prevImage && _this.render(_this.prevImage),
            _this.resizeQueued = false
          })
      ...
      var CompositeToCanvas = function () {
        ...
        return CompositeToCanvas.prototype.handleResize = function (e, w, h) {
          //debugger; wont run this function
    ...
    var Controller = function () {
      function Controller(source, destEl, assetConfig) {
        this.currentProgress = 0, this.lastFrameNumber = -1, this.playTimer = null, this.canJumpNow = !0, this.offset = 0, this.frameQueued = null, this.progressCallback = null, this.scrubTimer = null, this.currentVelocity = 0, this.source = source, this.length = this.source.length, this.el = destEl, this.renderer = assetConfig.rendererClass ? new SH.Video.Renderer[assetConfig.rendererClass](destEl) : new SH.Video.Renderer.ImageToCanvas(destEl), this.keyFrames = _.map(assetConfig.sections, function (el) {
          return el[2]
        }), this.keyFrames.unshift(0),this.chapters = _.map(assetConfig.chapters, function (el) {
          return el
        }), this.hiResDelay = assetConfig.hiResDelay || SH.Config.hiResDelay, this.currentProgress = (assetConfig.startFrame || 0) / (this.length - 1)
      }
...
! function (SH) {
  ! function (Input) {
    ...
    var EventTypes = Input.EventTypes,
      ...
      mousewheelTarget = $(window).add("html,body");
    ...
    var KeyboardHandler = function () {
      ...
      touchTarget = $("#full-content").add(window).add("html,body"),
      ...
      EventStream = function () {
        function EventStream() {
          this.acceptingInput = !1, this.handlers = [new KeyboardHandler(this)], this.handlers.push(Modernizr.hasEvent("touchstart") ? new TouchHandler(this) : new MouseWheelHandler(this))
        }
  ...
  ! function (Interface) {
    var Viewport = function () {
      ...
      return Viewport.prototype.update = function () {
        // console.log('Viewport.prototype.update()');
        var w = $(window),
            ww = w.width(),
            wh = w.height(),
            wa = ww / wh,
        ...
        scaleFactor = Math.min(dw / 1280, dh / 720);
        $(window).trigger("viewportResize", [dw, dh, ww, wh]);
...
! function (SH) {
  ! function (Interface) {
    ...
    var storyData = {
        "S01_STORY_1": {
          "title": "S03_STORY_1_title",
          "copy": "<p>S03_STORY_1_paragraph_candle</p>",
          "section": "candle",
          "jump_to_destination": 368,
          "className" : "STORY_1_1",
          "chapters": 3
        },
        "S01_STORY_2": {
          "title": "S03_STORY_2_title",
          "copy": "<p>S03_STORY_2_paragraph_eye</p>",
          "section": "eye",
          "jump_to_destination": 368,
          "className" : "STORY_1_2",
          "chapters": 1
        ...
        "S01_STORY_3": {
            "title": "S03_STORY_3_title",
            "copy": "<p>S03_STORY_3_paragraph_rose</p>",
            "section": "rose",
            "jump_to_destination": 368,
            "className" : "STORY_1_3",
            "chapters": 2
        ...
        return Story.getStory = function (id) {
          if ("undefined" == typeof storyCache[id]) {
            ...
            if (data) {
              ...
              $el.data("jump_to_destination",data.jump_to_destination),
              $el.data("section",data.section),
              $el.data("chapters",data.chapters),
              $el.addClass(data.className),
              tpl.parentNode.appendChild(el),
              storyCache[id] = new Story(id, el)
              if (data.delta) {
                $el.data('delta',data.delta)
              }
            } else {
          ...
          redkiss.stream.acceptingInput = false; //冻结滚轮事件
          var _this = this;
          if (openStory && openStory.hide(), openStory = this, !this.visible) {
            ...
            // var el = this.$el,
              ...
              // console.log(el.prop('class').split(' '));
              var section_name = el.data('section');
              var chapters_name = el.data('chapters');

              var chapter_obj = redkiss.playhead.videoController.chapters[chapters_name - 1];
              console.log('chapter_obj: ',chapter_obj);
              redkiss.playhead.videoController.source.appendToCustomSequence(chapter_obj.begin,chapter_obj.end);

              var delta = 0;
              if (el.data('delta')) {
                delta = parseInt(el.data('delta'));
              }
              var classNameList = el.prop('class').split(' ');
              var num = null;
              var clsName = '';
              for (var i = 0; i < classNameList.length; i++) {
                 if (classNameList[i].substring(0, 6) == "STORY_") {
                  //  num = classNameList[i].split('_')[1];
                  clsName = '.' + classNameList[i];
                 }
              }
              //zzz
  ...
  ! function (Interface) {
    var MobilePopup = function () {
      ...
      return MobilePopup.prototype.showContent = function (content) {
        ...
        }) : (this.currentOverlay = this.templateElement.clone().appendTo("body"), this.currentOverlay.find(".content").html(content.clone().show()), this.currentOverlay.css({
          opacity: 0
        }).show().transition({
          opacity: 1
  ...
  ! function (Interface) {
    ...
    var Overlays = function () {
      ...
      return Overlays.setCoords = function (el, xy) {
        ...
        var iconDiv = $(ev.currentTarget).find(".appstore_icon"),
          iconID = iconDiv[0].id;
  ...
  ! function (Interface) {
    var LocalNav = function () {
      function LocalNav(el, playhead, sections, offset, navColors) {
        ...
        this.displayAt = sections[0][2] + offset,
        this.createItems()
      }
...
//        })
        ...
        this.scrollThumb.style.backgroundColor = c || "#333",
        f == prev[2] && f < this.playhead.length - 1 - this.offset ? (f > 10 && 40 > f && (c = "black"), this.downButton[0].style.color = c || "white", this.downButton[0].style.borderColor = c || "white", this.resetDownButtonTimer()) : this.stopDownButtonTimer()
      }, LocalNav
    }();
  ...
  ! function (Interface) {
    var Intro = function () {
      ...
      return Intro.prototype.setProgressCallback = function (fun) {
        ...
        var _this = this;
        return "undefined" == typeof silent && (silent = !1), !silent && this.progressCallback && this.position < this.offset && this.progressCallback(this.position / this.length, this.position, this.length),
          requestAnimationFrame(function () {
    ...
    var fade = {
      ...
      PreloadAnimation = function () {
        function PreloadAnimation(el, hash) {
          ...
          ], this.keyFramesRun = {}, this.animationComplete = $.Deferred(), this.transitionTime = 10, this.delayTime = 10, location.search.match(/SPEEDY/) && (this.transitionTime = 0, this.delayTime = 0), el.addClass("pre-animation-state"), el.show(), this.queue.resolve()
        }
        return PreloadAnimation.prototype.queueAnimation = function (fun) {
          ...
          //每隔400ms 传入加载进度 progress, 与预设百分比做判断,动画 intro界面的段落文字
          // debugger;
          var _this = this;
          _.each(this.keyFrames, function (frame) {
            ...
            }), _this.keyFramesRun[frame[0]] = true)
          })
  ...
  ! function (Core) {
    ...
    var Playhead = function () {
      ...
      return Playhead.now = function () {
        ...
        // console.log('this.introController.offset:',this.introController.offset);
        ...
        "undefined" == typeof speedLimited && (speedLimited = !0),
         1 > frameNumber && (frameNumber = Math.round(frameNumber * (this.length - 1))),
         this.speedLimited = speedLimited,
         this.targetFrame = frameNumber,
         this.targetFromFrame = this.position,
         this.targetDistance = this.targetFrame - this.targetFromFrame,
         this.targetStartTime = Playhead.now(),
      ...
      Playhead.prototype.step = function (stopFrame) {
          ...
          if (null === this.targetFrame) {
            ...
            1.7 * framesToSlow >= framesRemaining && (shouldAccelerate = !1),
      ...
      }, Playhead.maxVelocity = 40, Playhead.minVelocity = 10, Playhead
    }();
  ...
  ! function (Core) {
    var ConfigLoader = function () {
      ...
      return ConfigLoader.prototype.loadConfig = function () {
        ...
        var d = $.Deferred();
        // "assets/" + this.assetVersion + "/" + name + "?v" + (new Date).getTime()
        return $.getJSON("assets/JSONs" + "/" + name, function (data) {
          d.resolve(data)
        }), d
      }, ConfigLoader
    }();
  ...
  ! function (Core) {
    var App = function () {
      ...
      return App.prototype.stateLoadConfig = function () {
          ...
          this.loader = new SH.Interface.Loader($("#loader")[0]),
          this.overlays = new SH.Interface.Overlays(this.config.overlays, this.config.sections, introController.offset),
          this.scrollbar = new SH.Interface.Scrollbar($(".scrollbar"), this.playhead),
          this.nav = new SH.Interface.LocalNav($("#section-nav"), this.playhead, this.config.sections, introController.offset, this.config.navColors);
          var preloadClasses = "";
          if (this.hash) {
          ...
          var startImageNumber = Math.max(0, this.startFrame - this.playhead.introController.offset),
            ...
            firstFrameReady = frameSource.loadFrame(startImageNumber, true);

          SH.Util.whenAll([firstFrameReady, preloaderReady]).then(function () {
            $("#loader").hide(),
            _this.stateStartPreloading(),
            _this.playhead.videoController.seekTo(startImageNumber, true),
            // 所有后续界面的容器淡入
        ...
        App.prototype.stateStartPreloading = function () {
          var _this = this;
          this.playhead.introController.seekTo(this.isTinyScreen ? this.tinyScreenOffset : 0, true),
          this.loader.events.on("loadProgress", $.proxy(this, "handleLoadProgress")),
          ...
          redkiss.playhead.seekTo(0),redkiss.playhead.start(),redkiss.playhead.playTo(65);
    ...
    var IFRAME_TEMPLATE = '<iframe src="//www.youtube.com/embed/__ID__?autoplay=1&controls=2&fs=1&modestbranding=1&rel=0&showinfo=1&color=white&enablejsapi=1" frameborder="0" allowfullscreen id="youtube_player"></iframe>',
      VideoViewer = function () {
        ...
        return VideoViewer.prototype.openVideo = function (filename) {
          ...
          return this.layoutVideoElements(), $(window).bind("resize", $.proxy(this, "onResize")), this.currentOverlay.css({
            display: "block"
          }).transition({
            opacity: 1
          }), this.currentOverlay
          ...
          var _this = this,
            d = $.Deferred();
          return $(".video_replay_link").off("click", $.proxy(this, "onVideoReplayClicked")), $(window).unbind("resize", $.proxy(this, "onResize")), this.currentOverlay.transition({
            opacity: 0
          }, function () {
      ...
      function createShares() {
        ...
        _.each(shareData, function (val, key) {
          var node = tpl.clone().attr({
            ...
            twitterText = val.twitter || encodeURIComponent(title.replace(/[\r\n\s\t]+/g, " ")),
            twitterUrl = "https://twitter.com/intent/tweet?url=" + Social.SITE_ROOT + encodeURIComponent(val.link) + "&text=" + twitterText;
          node.find(".f").attr({
